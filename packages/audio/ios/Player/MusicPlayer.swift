...
class MusicPlayer: MusicPlayerContract {
    ...
    func start(
        list: [SongModelContract],
        currentIndex: Int,
        repeatMode: RepeatMode,
        shuffleMode: ShuffleMode,
        sendPlaybackSongChange: @escaping (SongModelContract) -> Void,
        sendIsPlayingChange: @escaping (Bool) -> Void
    ) {
        ...
        let items = MPMediaItemCollection(
            items: list.map { $0.media.representativeItem! as MPMediaItem }
        )
    ...
    func getCurrentSong() -> SongModelContract? {
        ...
        return SongModel(
            media: MPMediaItemCollection(items: [
                player.nowPlayingItem! as MPMediaItem
            ])
        )
    }
    ...
    func setRepeat(repeatMode: RepeatMode) {
        player.repeatMode =
            ...
            ? MPMusicRepeatMode.none
            : repeatMode == RepeatMode.one
                ? MPMusicRepeatMode.one
    ...
    func setShuffle(shuffleMode: ShuffleMode) {
        player.shuffleMode =
            ...
            ? MPMusicShuffleMode.off : MPMusicShuffleMode.songs
    }
