...
import EmptyRowListItem from '@/components/molecules/listItem/EmptyFooter';

const AlbumsPagePresenter = () => (
  <PageLayout>
    <FlatList data={[<EmptyHeader key={0} />, <AlbumList key={1} />, <EmptyRowListItem key={2} />]} renderItem={({ item }) => <>{item}</>} />
    <MiniPlayer />
  </PageLayout>
);
